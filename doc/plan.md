...
对左右 split，只沿 x 轴采样，成对对象的 y 坐标对齐到同一个 off-center lane；对上下 split，只沿 y 轴采样，成对对象的 x 坐标对齐到同一个 off-center lane，避免把左右题看成上下题或反过来；
inner pair 和 outer pair 在同一个 panel 中共享同一种二分方向；inner/outer 对象必须同时满足语义轴偏移和正交轴 off-center 偏移，不能挤在 boundary 中线附近；
...
region position = sampled only along the split axis; the cross-axis is aligned to an off-center lane
